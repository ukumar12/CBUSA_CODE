...
Partial Class skuprice
    ...
    Private Sub GetFilterList(ByRef list As Generic.List(Of String))
        If m_FilterList Is Nothing Then
            ...
            Select Case rblFilter.SelectedValue
                Case "vendor"
                    SQL = "select ProductID from VendorProductPrice where VendorID=" & DB.Number(VendorId) & " and (VendorPrice is not null or ProductID in (select ProductID from VendorProductSubstitute where VendorID=" & DB.Number(VendorId) & "))"
                Case "phases"
                    SQL = _
                        " with Phases(SupplyPhaseId) as (" _
                        & "     select SupplyPhaseId from SupplyPhase where ParentSupplyPhaseId is not null and SupplyPhaseId in (select SupplyPhaseId from VendorSupplyPhase where VendorId=" & DB.Number(VendorId) & ")" _
                        & "     union all " _
                        & "     select sp.SupplyPhaseId from SupplyPhase sp inner join Phases p on sp.ParentSupplyPhaseId=p.SupplyPhaseId" _
                        & " )" _
                        & " select ProductId from SupplyPhaseProduct spp where spp.SupplyPhaseId in (select distinct SupplyPhaseId from Phases)" _
                        & " and SupplyPhaseId not in (select ex.SupplyPhaseId from SupplyPhaseLLCExclusion ex inner join LLCVendor l on ex.LLCId=l.LLCId where l.VendorId=" & DB.Number(VendorId) & ")"
                ...
                Select Case rblFilter.SelectedValue
                    ...
                    Case "phases"
                        sql = _
                            ...
                            & "     select sp.SupplyPhaseId from SupplyPhase sp inner join Phases p on sp.ParentSupplyPhaseId=p.SupplyPhaseId" _
                            & " )" _
                            & " select Distinct SupplyPhaseId from Phases where SupplyPhaseId not in (select ex.SupplyPhaseId from SupplyPhaseLLCExclusion ex inner join LLCVendor l on ex.LLCId=l.LLCId where l.VendorId=" & DB.Number(VendorId) & ")"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ScriptManager.GetCurrent(Page).RegisterAsyncPostBackControl(rblSubPricedOnly)
        If VendorId = 0 Then
        ...
        Core.DataLog("Update Skus and Pricing", PageURL, CurrentUserId, "Vendor Top Menu Click", "", "", "", "", UserName)

            ctlSearch.KeywordsTextboxId = txtKeywords.UniqueID
            ...
            ctlSubSearch.KeywordsTextboxId = txtKeywords.UniqueID
            ...
            ctlSearch.SetCatalogType(controls_SearchSql.CATALOG_TYPE.CATALOG_TYPE_VENDOR)
    ...
    Protected Sub ctlSearch_ResultsUpdated(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctlSearch.ResultsUpdated
        rptProducts.DataSource = From sr As DataRow In ctlSearch.SearchResults.AsEnumerable _
    ...
    Private Sub BindProducts()
        ...
        End If
        If txtKeywords.Text <> String.Empty And txtKeywords.Text <> "Keyword Search" Then
            SQL &= " And ("
            SQL &= "p.Product Like " & DB.FilterQuote(txtKeywords.Text)
            SQL &= " Or p.Description Like " & DB.FilterQuote(txtKeywords.Text)
            SQL &= " Or p.SKU = " & DB.Quote(txtKeywords.Text)
            SQL &= ")"
            ltlBreadcrumb.Text = "<a href=""/"">Home</a> > <a href=""sku-price.aspx?sp=N"">All Supply Phases</a> > " & txtKeywords.Text
        ...
        btnImportUpdateTop.Visible = False
        btnImportPriceUpdateTop.Visible = False
    End Sub
    ...
    Protected Sub rptProducts_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptProducts.ItemCommand
        ...
        If txtCurrentPrice.Text <> String.Empty Then
            If Not IsNumeric(txtCurrentPrice.Text) And txtCurrentPrice.Text > 0 Then
                ErrorMsg &= "<li>Please enter a valid Current Price for the selected product"
            ElseIf Math.Round(Double.Parse(txtCurrentPrice.Text), 2) <= 0 Then
                ErrorMsg &= "<li>Please enter a valid Current Price for the selected product"
            ...
            '    Dim dbExistingProduct As VendorProductPriceRow = VendorProductPriceRow.GetRowByVendorSku(DB, VendorId, txtSku.Text)
            '    If dbExistingProduct.Submitted <> Nothing AndAlso VendorPrice <> Double.Parse(txtCurrentPrice.Text) Then
            '        ErrorMsg &= "<li>Products with sames SKU must have same Price"
            '    End If
            'End If
        ...
        If txtNextPrice.Text <> String.Empty Then
            ...
            ElseIf Math.Round(Double.Parse(txtNextPrice.Text), 2) <= 0 Then
                ErrorMsg &= "<li>Please enter a valid Next Price for the selected product"
            End If
        End If
    ...
    Protected Sub rptProducts_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptProducts.ItemDataBound
        ...
        Dim chkDiscontinue As CheckBox = e.Item.FindControl("chkDiscontinue")
    ...
    Protected Sub btnImportUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnImportUpdate.Click, btnImportUpdateTop.Click
        Try
    ...
    Protected Sub btnImportPriceUpdate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnImportPriceUpdate.Click, btnImportPriceUpdateTop.Click
        Try
    ...
    Private Sub ImportCSV(ByVal FileName As String, ByVal UpdateDatabase As Boolean)
        ...
        While Not f.EndOfStream
            ...
            If aLine.Length > 3 AndAlso aLine(2) <> String.Empty Then
                IsUpdate = False
                ProductId = 0
                SKU = Trim(Core.StripDblQuote(aLine(2)))
                    ...
                    If IsUpdate Then
                        ...
                        If VendorPrice <> String.Empty Then
                            ...
                            Else
                                ...
                                SQL &= ",NextPriceApplies = " & DB.Quote(ProductRow.GetPriceLockDate(DB, ProductId))
                            End If
                        End If
                        SQL &= ",SubstituteQuantityMultiplier = " & DB.Number(0)
        ...
        Else
            ...
            btnImportUpdateTop.Visible = True
    ...
    Private Sub ImportPriceCSV(ByVal FileName As String, ByVal UpdateDatabase As Boolean)
        ...
        While Not f.EndOfStream
            ...
            If aLine.Length > 1 AndAlso aLine(0) <> String.Empty Then
                ...
                If IsNumeric(VendorPrice) Then
                    VendorPrice = Math.Round(Double.Parse(VendorPrice), 2)
                End If
                If Count = 1 And VendorSKU.ToUpper <> "VENDOR SKU" Then
                    ltrErrorMsg.Text = "<table><tr><td><img src=""/images/exclam.gif"" border=""0""/></td>"
                    ltrErrorMsg.Text &= "<td valign=""middle""><span class=""red bold""><li>The file you uploaded does not appear to be in the correct format. The file format must be identical to the one used in the Export CSV process.</td></tr></table>"
                    Exit Sub
                End If

                If VendorSKU.ToUpper = "VENDOR SKU" Then
                    Continue While
                End If
                ...
                End If
                If txtErr = String.Empty Then
                    Dim SQL As String = "SELECT p.ProductID, p.Product, p.SKU, vpp.VendorSKU, Cast(vpp.VendorPrice As Money) As VendorPrice, vpp.IsSubstitution, vpp.IsDiscontinued, vpp.Updated "
                    SQL &= "FROM Product p INNER JOIN "
                        ...
                        For Each dr As DataRow In dtProduct.Rows
                            ...
                            If txtErr = String.Empty And UpdateDatabase Then
                                ...
                                Try
                                    ...
                                    If UpdateCurrent And Not IsDBNull(dr("VendorPrice")) Then
                                        Dim dbHistory As New VendorProductPriceHistoryRow(DB)
                                        dbHistory.IsSubstitution = False
                                        dbHistory.IsUpload = False
                                        dbHistory.ProductID = ProductId
                                        dbHistory.SubmitterVendorAccountID = Session("VendorAccountId")
                                        dbHistory.VendorID = Session("VendorId")
                                        dbHistory.VendorPrice = dr("VendorPrice")
                                        dbHistory.VendorSKU = dr("VendorSKU")
                                        dbHistory.Insert()
                                    End If
        ...
        Else
            ...
            btnImportPriceUpdateTop.Visible = True
            ltrErrorMsg.Text = "<b>File uploaded successfully. Please review the report below and confirm the data import.</b>"
        End If
    ...
    Protected Sub btnSaveAll_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveAll1.Click, btnSaveAll2.Click
        ...
        Try
            For Each item As RepeaterItem In rptProducts.Items
                ...
                If txtSku.Text <> Nothing Or txtCurrentPrice.Text <> Nothing Or txtNextPrice.Text <> Nothing Then
                    ...
                    Dim PreviousSKU As String = String.Empty
                    Dim PreviousVendorPrice As String = String.Empty
                    ...
                    If price = Nothing And txtCurrentPrice.Text <> Nothing Then
                        ...
                        Continue For
                    ElseIf price = String.Empty Then
                        AddError("Price is Invalid (Row #" & cnt & ")")
                        Continue For
                    ElseIf p.VendorPrice <> Nothing AndAlso (price = Nothing OrElse Convert.ToDouble(price) > p.VendorPrice) Then
                        ...
                        '    Dim dbExistingProduct As VendorProductPriceRow = VendorProductPriceRow.GetRowByVendorSku(DB, VendorId, sku, p.ProductID)
                        '    If dbExistingProduct.Submitted <> Nothing AndAlso dbExistingProduct.VendorPrice <> price Then
                        '        AddError("Products with same Vendor SKU must have same Price")
                        '        Continue For
                        '    End If
                    End If
                    ...
                    If txtCurrentPrice.Text <> Nothing Then
                        If p.VendorPrice <> price Then
                            UpdateHistory = True
                        End If
                        PreviousVendorPrice = p.VendorPrice
                    ...
                    If UpdateHistory Then
                        ...
                        dbHistory.SubmitterVendorAccountID = Session("VendorAccountId")
                        dbHistory.VendorID = Session("VendorId")
                        dbHistory.VendorPrice = PreviousVendorPrice
                        dbHistory.VendorSKU = PreviousSKU
            ...
            DB.CommitTransaction()
            'log Btn Update Pricing for All Product SKUs
            Core.DataLog("Edit Supply Phases", PageURL, CurrentUserId, "Btn Update Pricing for All Product SKUs", "", "", "", "", UserName)
            'end log
...
#Region "Substitutions"
    ...
    Protected Sub frmSelect_TemplateLoaded(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        CType(form.FindControl("ltlProductName"), Literal).Text = item.DataItem.Product
    End Sub
    ...
    Private Sub BindSearchData(ByVal tbl As DataTable, ByVal count As Integer)
        ...
        If rblSubPricedOnly.SelectedValue = True Then
            Dim q = From sr As DataRow In tbl.AsEnumerable _
                    Join vp As DataRowView In dvVendorPrices _
                    On Convert.ToInt32(sr("ProductId")) Equals Convert.ToInt32(vp("ProductId")) _
                    Where Not IsDBNull(vp("VendorPrice")) _
                    Select New With _
                    { _
                        .ProductID = sr("ProductId"), _
                        .VendorSku = vp("VendorSku"), _
                        .ProductSku = sr("SKU"), _
                        .Product = sr("Product") _
                    }
        ...
        Else
            ...
            ctlSubNavigator.Visible = True
            rptResults.Visible = True
            rptResults.DataBind()
        End If
    End Sub

    Protected Sub ctlSubSearch_OnTreeNodeSelect(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctlSubSearch.OnTreeNodeSelect
        Dim list As New Generic.List(Of String)
        SubFilterListCallback(list)
        ctlSubSearch.FilterList = list
    End Sub
    ...
    Protected Sub ctlSubSearch_ResultsUpdated(ByVal sender As Object, ByVal e As System.EventArgs) Handles ctlSubSearch.ResultsUpdated
        BindSearchData(ctlSubSearch.SearchResults, ctlSubSearch.SearchResults.Rows(0).Item(3))
    ...
    Protected Sub SubFilterListCallback(ByRef list As Generic.List(Of String))
        list.AddRange(From vp As DataRowView In dvVendorPrices Where Not IsDBNull(vp("VendorPrice")) And Not Core.GetBoolean(vp("IsSubstitution")) And Core.GetString(vp("ProductID")) <> hdnProductID.Value Select Convert.ToString(vp.Item("ProductId")))
    End Sub
...
#End Region
    ...
    Protected Sub frmConfirmDiscontinue_Postback(ByVal sender As Object, ByVal e As System.EventArgs) Handles frmConfirmDiscontinue.Postback
        Try
            ...
            Dim dbHistory As New VendorProductPriceHistoryRow(DB)
            'update vendorprice history record
            Dim dbProduct As VendorProductPriceRow = VendorProductPriceRow.GetRow(DB, Session("VendorId"), hdnDiscontinueProductID.Value)
            dbHistory.IsSubstitution = False
            dbHistory.IsUpload = False
            dbHistory.ProductID = dbProduct.ProductID
            dbHistory.SubmitterVendorAccountID = Session("VendorAccountId")
            dbHistory.VendorID = Session("VendorId")
            dbHistory.VendorPrice = dbProduct.VendorPrice
            dbHistory.VendorSKU = dbProduct.VendorSKU
            dbHistory.Insert()
        ...
        End Try
    End Sub
    Protected Sub btnDiscontinueSelected_Click(sender As Object, e As System.EventArgs) Handles btnDiscontinueSelected1.Click, btnDiscontinueSelected2.Click
        DB.BeginTransaction()
        Try
            For Each item As RepeaterItem In rptProducts.Items
                Dim chkDiscontinue As CheckBox = item.FindControl("chkDiscontinue")
                Dim btnUpdate As Button = item.FindControl("btnUpdate")
                If chkDiscontinue.Checked Then

                    Dim dbHistory As New VendorProductPriceHistoryRow(DB)
                    'update vendorprice history record
                    Dim dbProduct As VendorProductPriceRow = VendorProductPriceRow.GetRow(DB, Session("VendorId"), btnUpdate.CommandArgument)
                    dbHistory.IsSubstitution = False
                    dbHistory.IsUpload = False
                    dbHistory.ProductID = dbProduct.ProductID
                    dbHistory.SubmitterVendorAccountID = Session("VendorAccountId")
            ...
            Next
            DB.CommitTransaction()
            'log Btn Discontinue Pricing for All Selected Products
            Core.DataLog("Edit Supply Phases", PageURL, CurrentUserId, "Btn Discontinue Pricing for All Selected Products", "", "", "", "", UserName)
            'end log
            UpdateSearch()
        Catch ex As Exception
            DB.RollbackTransaction()
    ...
    Private Sub UpdateSearch(Optional ByVal pIgnoreSelectedNode As Boolean = False)

        Select Case rblFilter.SelectedValue
            Case "vendor"
                ctlSearch.SetCatalogType(controls_SearchSql.CATALOG_TYPE.CATALOG_TYPE_VENDOR)
            Case "market"
                ctlSearch.SetCatalogType(controls_SearchSql.CATALOG_TYPE.CATALOG_TYPE_MARKET)
            Case "all"
                ctlSearch.SetCatalogType(controls_SearchSql.CATALOG_TYPE.CATALOG_TYPE_ALL)
        End Select

        ctlSearch.SearchProduct(0, True, pIgnoreSelectedNode)
