    ...
    div.appendChild(result);
        ...
        if(i == resultArray1.length -1) {
            ...
            for(var j = 0; j < ctrls.length; j++) {
                if(ctrl == null && ctrls[j].id.indexOf('txtKeyword') > -1) {
                    ctrl = ctrls[j];
                }
            }
            if(ctrl) {
                ...
                footer.innerHTML = '<a href="javascript:void(0);" onmousedown=\'window.location="/search/default.aspx?keywords=' + escape(ctrl.value) + '"\'>View All Matches</a>' + (parseInt(count) >= 0 ? '<div style="margin-top:3px;font-size:.85em;">' + count + ' result(s)</span>' : '');
