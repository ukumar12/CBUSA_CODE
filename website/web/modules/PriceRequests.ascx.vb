    ...
    Protected Sub rptRequests_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptRequests.ItemCommand
        Select Case e.CommandName
            Case "Remove"
                DB.BeginTransaction()
                Try
                    Dim dbRequest As VendorProductPriceRequestRow = VendorProductPriceRequestRow.GetRow(DB, e.CommandArgument)
                    ...
                    MsgBody = dbVendor.CompanyName & " has deleted your Price Request for the product: " & vbCrLf & vbCrLf & ProductRow.GetRow(DB, dbRequest.ProductID).SKU & " - " & ProductRow.GetName(DB, dbRequest.ProductID) & vbCrLf & " because they do not carry this product or a suitable subsitute. " & vbCrLf & vbCrLf & "Please contact the preferred vendor directly for clarification."
            ...
            Case "Save"
                ...
                Try
                    ...
                    If price = String.Empty OrElse Double.Parse(price) <= 0 Then
                        ...
                        Exit Sub
                        'ElseIf txtSku.Text <> String.Empty Then
                        '    Dim dbExistingProduct As VendorProductPriceRow = VendorProductPriceRow.GetRowByVendorSku(DB, Session("VendorId"), txtSku.Text)
                        '    If dbExistingProduct.Submitted <> Nothing AndAlso dbExistingProduct.VendorPrice <> Double.Parse(price) Then
                        '        ctlError.AddError("Price must match other products with this SKU")
                        '        ctlError.Visible = True
                        '        Exit Sub
                        '    End If
                    End If
