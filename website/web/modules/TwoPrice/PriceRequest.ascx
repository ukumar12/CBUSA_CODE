<%@ Control Language="VB" AutoEventWireup="false" CodeFile="PriceRequest.ascx.vb" Inherits="modules_TwoPrice_PriceRequest" %>
...
<script type="text/javascript">
    ...
    function twoPriceOpenSubstitute(id, name) {
        ...
        var hdnSubstitute = $get('<%=hdnSubstituteProductName.ClientID %>');
        hdnSubstitute.value = name;
    ...
    function twoPriceSwapForms() {
        Sys.Application.remove_load(twoPriceSwapForms);
        twoPriceCloseSubForm();
        twoPriceOpenSelectForm();
    }
    function twoPriceCloseSubForm() {
        Sys.Application.remove_load(twoPriceCloseSubForm);
        var b = Sys.UI.Behavior.getBehaviorByName($get('<%=frmSubstitute.BehaviorId %>'), '<%=frmSubstitute.BehaviorName %>');
        b.fadeOut();
        return true;
    }
    function twoPriceCloseSubForm2() {
        Sys.Application.remove_load(twoPriceCloseSubForm);
        var b = Sys.UI.Behavior.getBehaviorByName($get('<%=frmSubstitute.BehaviorId %>'), '<%=frmSubstitute.BehaviorName %>');
        b.fadeOut();
        Sys.WebForms.PageRequestManager.getInstance().add_pageLoaded(twoPriceCloseSubForm);
        return true;
    }
    function twoPriceOpenSelectForm() {
        var frm = $get('<%=frmSelect.ClientID %>').control;
        //frm._doMoveToCenter();
        frm.Open();
    }
    function twoPriceCloseSelectForm() {
        Sys.Application.remove_load(twoPriceCloseSelectForm);
        var frm = $get('<%=frmSelect.ClientID %>').control;
        frm.Close();
        return true;
    }
    function twoPriceUpdateTotal(e) {
                        ...
                        <table border="0" cellpadding="5" cellspacing="0">
                            ...
                            <tr>
                                <td colspan="3" style="text-align:center;">
                                    <asp:Button ID="btnSave" runat="server" Text="Save Substitute" CssClass="btnred" OnClientClick="twoPriceCloseSubForm2();"  onclick="frmSelect_Postback"/>&nbsp;
                                    <asp:Button ID="btnCancel" runat="server" Text="Cancel" CssClass="btnred" OnClientClick="twoPriceCloseSubForm2();" />
...
<CC:DivWindow ID="frmSubstitute" runat="server" TargetControlID="divSubstitute" CloseTriggerId="btnCancelSub" ShowVeil="false" VeilCloses="true" />
    <div id="divSubstitute" runat="server" class="window" style="width:900px;">
        <div class="pckggraywrpr" style="margin-bottom:0px;">
            <div class="pckghdgred">Substitute Product</div>
            <div >
                        ...
                        <td>
                            ...
                            <div class="pckghdgblue autoHeight">
                                <div style="float:left;margin-right:100px;">
                                    ...
                                    <asp:HiddenField ID="hdnSubstituteProductName" runat="server" Value="" />
                                </div>
...
<div class="pckgwrpr">
    <div class="pckghdgltblue">Committed Purchases Price Requests</div>
        ...
        <asp:Repeater ID="rptRequests" runat="server">
            ...
            <ItemTemplate>
                <tr id="trInfo" runat="server" valign="top">
                    ...
                    <td><asp:Button id="btnSubstitute"   runat="server" CausesValidation="false" text="Substitute" cssclass="btnred" OnClientClick='<%# "return twoPriceOpenSubstitute(""" & Convert.ToString(DataBinder.Eval(Container.DataItem,"TwoPriceVendorProductPriceRequestId")) &""","""& convert.tostring(dataBinder.Eval(Container.dataItem,"Product")).Replace("""","\""") &""");" %>'></asp:Button></td>
                    ...
                    <td align="right"><CC:ConfirmImageButton Message="Are you sure you want to delete this request?" ID="btnRemove" runat="server" AlternateText="Remove" ImageUrl="/images/global/icon-remove.gif" CommandName="Remove" CommandArgument='<%#DataBinder.Eval(Container.DataItem,"TwoPriceVendorProductPriceRequestId") %>' /></td>
                </tr>
                <tr id="trForm" runat="server" valign="bottom">
                  ...
                  <tr id="trFormApplies" runat="server">
                                <td  >Applies To:</td>
                                

                                <td>
                                    <asp:RadioButton ID="rbFormAppliesAlways" runat="server" GroupName="PriceRequests" Checked="true" text="All Requests For This Product" /><br />
                                   </td>
                                  <td>   <asp:RadioButton ID="rbFormAppliesOnce" runat="server" Text="This Builder's Request Only" GroupName="PriceRequests" />
                                </td>
