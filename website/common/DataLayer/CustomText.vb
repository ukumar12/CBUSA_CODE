...
Namespace DataLayer
    ...
    Public Class CustomTextRow
        Inherits CustomTextRowBase
        ...
        Public Shared Function GetRow(ByVal DB As Database, ByVal TextId As Integer) As CustomTextRow
            Dim row As CustomTextRow

            row = New CustomTextRow(DB, TextId)
            row.Load()
        ...
        'Custom Methods
        Public Shared Function GetRowByCode(ByVal DB As Database, ByVal Code As String) As CustomTextRow
            Dim SQL As String = "SELECT * FROM CustomText WHERE Code = " & DB.Quote(Code)
            Dim r As SqlDataReader
            Dim row As CustomTextRow = New CustomTextRow(DB)
            r = DB.GetReader(SQL)
            If r.Read Then
                row.Load(r)
            End If
    ...
    Public MustInherit Class CustomTextRowBase
        ...
        Protected Overridable Sub Load()
            ...
            SQL = "SELECT * FROM CustomText WHERE TextId = " & DB.Number(TextId)
        ...
        Public Sub Remove()
            ...
            SQL = "DELETE FROM CustomText WHERE TextId = " & m_DB.Quote(TextId)
            m_DB.ExecuteSQL(SQL)
        End Sub 'Remove
    End Class

    Public Class CustomTextCollection
        Inherits GenericCollection(Of CustomTextRow)
    End Class
